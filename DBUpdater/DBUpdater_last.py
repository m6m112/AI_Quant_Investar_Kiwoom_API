...
class DBUpdater:
    ...
    def read_naver(self, code, company, pages_to_fetch):
        ...
        try:
            ...
            html = BeautifulSoup(requests.get(url, headers = {'User-agent' : 'Mozilla/5.0'}).text,"lxml")                   #요청한 url 가져옴, parser --> lxml (html을 해석하는 패키지)
            ...
            for page in range(1, pages + 1):                                        #페이지 수 만큼 반복
                ...
                df = df.append(pd.read_html(requests.get(pg_url, headers = {'User-agent' : 'Mozilla/5.0'}).text)[0])                   #각 page를 df에 저장
...
if __name__ == '__main__':                      #main 함수를 의미
    ...
    dbu.execute_daily()                      #dbu객체의 execute_daily 함수 실행 
